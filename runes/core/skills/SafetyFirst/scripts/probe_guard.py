...
def wait_unreaped(process, timeout):
    ...
    An unreaped leader stays a zombie, so its pid still names this group when
    stop_group runs, and its recorded exit status survives the kill.
    """
...
def stop_group(process):
    ...
    Runs before anything reaps the leader, so the group id cannot have been
    recycled. On macOS a group whose only member is an unreaped zombie answers
    EPERM, and an already reaped leader answers ESRCH; both mean there is
    ...
    try:
        with selectors.DefaultSelector() as selector:
            ...
            if not outcome["timed_out"] and not outcome["truncated"]:
                try:
                    wait_unreaped(process, max(0, deadline - time.monotonic()))
