...
class ConfigTests(unittest.TestCase):
    ...
    def test_score_version_and_filler_category(self):
        ...
        self.assertEqual(result["score_version"], 6)
...
"""
    ...
    def test_unterminated_code_fence_marks_density_unreliable(self):
        result = STE_LINT.lint("```text\nThis hidden text has many words but no closing fence.")

        self.assertFalse(result["density_reliable"])
        self.assertEqual(result["density_note"], "Advisory: unterminated Markdown structure.")

    def test_unterminated_frontmatter_marks_density_unreliable(self):
        result = STE_LINT.lint("---\ntitle: Example\nThis text has no closing marker.")
