...
- **Should-trigger (10)**: different phrasings of the same intent, formal and casual. Cases where the user never names the skill or file type but clearly needs it. Uncommon use cases. Cases where this skill competes with another but should win.
- **Should-not-trigger (10)**: near-misses that share keywords or concepts but need something different: adjacent domains, ambiguous phrasing a naive keyword match would catch, contexts where another tool is more appropriate. Obviously irrelevant negatives test nothing.
