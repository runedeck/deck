...
def main():
    ...
    parser.add_argument("--report", default="none", help="Generate a live HTML report: 'none' to disable (default), 'auto' for a temp file, or a path")
    parser.add_argument("--results-dir", default=None, help="Save all outputs (results.json, report.html, log.txt) to a timestamped subdirectory here")
    args = parser.parse_args()

    if not (args.holdout == 0 or 0 < args.holdout < 1):
        parser.error("--holdout must be 0 (disabled) or strictly between 0 and 1")
    ...
    # Set up live report path (only when explicitly requested)
    if args.report != "none":
        if args.report == "auto":
