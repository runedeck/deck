...
- [ ] No H2 falls outside the Stable shell vocabulary.
...
- [ ] Standalone `mdschema` validates the entrypoint when strict Stable shell validation is required.

Rune's built-in schema checker is a partial fallback. It checks basic structure but does not replace standalone `mdschema` for section vocabulary, ordering, uniqueness, or subsection placement.
...
- [ ] The entrypoint and Markdown companions satisfy the ArtifactLength rule.
