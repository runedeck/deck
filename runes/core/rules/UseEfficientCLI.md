...
Use `sd` for find-and-replace edits. Run `sd --version` first. When the output is not the find-and-replace tool, call the path that `command -v -a sd` lists for it. Read a file region with an available file-reading tool. If none is available, use a bounded shell read. Page JSON or YAML output with `jless` when you must explore it. Compare code structurally with `difft` (`jj diff --tool difft`) when a line diff drowns the change.

The shell may run with `noclobber` set. A plain `>` or `2>` redirect to an existing file then fails (zsh prints `file exists`, bash prints `cannot overwrite existing file`). Use `>|` and `2>|` when you intend to overwrite. Use `>>` when you intend to append. Do not disable the option.
