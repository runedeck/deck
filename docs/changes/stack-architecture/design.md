...
- **Ceremony** (workflows, hooks, labels, `authors.yaml`) enforces both. Its source of truth is the skeleton; consumers receive it through Copier updates.
...
| Deck | `runedeck/deck` and sibling repos | reviewed, proven, schema-valid artifacts and their sidecars | rendered personal values; review transcripts (DECK-0002 Temporary Adoption State) |
...
- **Consumer**: remove the manifest entry; `rune clean` and prune remove the deployed files the deploy manifest tracks.
...
Reality diverges from this abstraction in the following places. This change records them; it fixes none of them.

1. **Ceremony shipping lag.** The deck's Copier pin trails the skeleton by 24 `templates/base` commits; the label grammar splits the fleet (`spec:none` in deck, cli, seer; `ignore:spec` in skeleton). Seam: the governance plane's Ship stage is manual.
...
4. **Unenforced hops.** Nothing blocks a rule from shipping without a bench verdict, or an adopted artifact from merging without a sealed record. The tokens exist; the gates do not.
...
6. **Provider routine install is manual by nature.** Provider UIs accept pasted prompts. The CLI can render and verify; it cannot install. This is an accepted edge, not a fixable seam.
...
8. **Workshop capture is convention-only.** CaptureOnTouch is a rule; nothing measures whether capture happens. The Operate-to-Capture feedback loop has no coverage signal.
