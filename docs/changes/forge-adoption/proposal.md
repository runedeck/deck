...
- One rule in `runes/core/rules/`, RemoteWrites: a command list, never `gh pr merge|close|comment|review|edit --add-label|--remove-label`, `gh issue comment|close`, `git push`, `jj git push`, or a thread resolution, with two exceptions for the invoking agent: the review label the prompt names, and one correctness summon per green head on a pull request it opened or babysits. Always on. A three-model review was unanimous that this cannot live in a skill.
